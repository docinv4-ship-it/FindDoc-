...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!doctor_id || !clinic_id || !patient_name || !patient_phone) return NextResponse.json({ error: "Doctor ID, clinic ID, patient name, and phone are required" }, { status: 400 });
    ...
    if (existingPatient) {
      ...
      if (patientError) { console.error("Error creating patient:", patientError); return NextResponse.json({ error: "Failed to create patient" }, { status: 500 }); }
    ...
    if (convoError) { console.error("Error creating conversation:", convoError); return NextResponse.json({ error: "Failed to create conversation" }, { status: 500 }); }

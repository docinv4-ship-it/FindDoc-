...
import { v4 as uuidv4 } from "uuid"; // Ensure you have `uuid` installed: npm i uuid @types/uuid
...
export default function AvailabilityStep({ data, onChange, errors }: AvailabilityStepProps) {
  ...
  const addSlot = (dayName: string) => {
    const day = data.schedule.find(d => d.day === dayName);
    if (!day) return;
    
    const newSlot: TimeSlot = { id: uuidv4(), startTime: "09:00", endTime: "17:00" };
